...
class ModelDetected:
    ...
    def load_video(self, filename: str, input_file: str, output_file: str):
        ...
        if self.type_cam == 'front':
            if self.width > self.height:
                ...
                raise "Определить границы для вертикального видео"
        elif self.type_cam == 'left':
            raise "Определить границы для камер с левой стороны"
        else:
            raise "Определить границы для камер с правой стороны"
    ...
    def create_history_df(self, history):
        ...
        self.history_df['time'] = self.history_df['time'].apply(
            lambda x: '\"{0:02d}:{1:02d}\"'.format(int(x // 60), int(x - int(x // 60) * 60)))
